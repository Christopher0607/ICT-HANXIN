...
政策問清楚了（Topstep 2026-09-10 書面答覆，全文在
`docs/topstep_automation_enquiry.md`）：
...
TradingView 的警報是從**它的伺服器**推出來的，要進到你家的電腦得再架一層隧道。
既然訊號引擎（`strategies/ltf_sweep.py`）本來就在本機、
K 棒也可以從 TopstepX 自己拿（`POST /api/History/retrieveBars`），
整條 TradingView → 隧道 → 本機的路就沒有存在的理由了 ——
還省下 Essential $12.95/月 加 CME 即時行情 $7/月。

**現在下單的是 `bridge/live.py`，用的是回測跑的那一份 Python 引擎。**
Pine 這兩支腳本從此只做兩件事：**看圖**，以及**人眼交叉驗證**。
它們不再需要跟 Python 保持逐行一致 —— 那整類走鐘風險消失了。

上線步驟看 `docs/GO_LIVE.md`。
...
**一、這套策略十一年累計是虧的。**
NQ 2016–2026、2,161 筆交易，淨損益 **−$3,044**，11 年裡 **5 年虧損**，
而且**累計損益從來沒有轉正過**。2023 之後才連續為正
（2023 +$12,792、2024 +$6,816、2025 只有 +$658、2026 +$14,483）。

把 11 年切成所有的三個月視窗（126 個），**中位數獲利因子 0.996 ——
也就是中位數的三個月是打平**，只有 49.2% 的視窗獲利，
而當前視窗的 2.06 落在第 **98** 百分位。

這既可能是 2023 後市場真的變了，也可能只是十一年裡最好的那一段而我們正在看它。
資料本身分不出來 —— 只有 2026-08-28 之後的新 K 棒能分
（`scripts/forward_log.py`）。

**這些數字每一個都可以重跑**：`scripts/run_ltf_sweep.py` 出總表，
`scripts/run_backtest.py --by-year` 出逐年，
`backtest/metrics.py` 的 `rolling_windows()` / `window_percentile()` 出視窗分布。
...
|---|---|---|
| 用途 | **看 Strategy Tester、驗證邏輯** | 盤中看訊號標記 |
...
**兩份都不接自動化了**（見上面那段）。日常用 strategy 版就好 ——
它有損益報表，而 indicator 版的 `alert()` 現在沒有東西在接。
...
**圖表左上角的圖例會顯示版本號**：`LTF Sweep v6`。
看到版本號就是新版，只看到 `LTF Sweep` 就是舊的還掛著 ——
不用開面板、不用翻 log，你截圖的時候本來就會拍到。

`debugLog` 打開的話，Pine Logs 第一行也會印 `LTF Sweep v6`。
...
**別把這個跟同一根 K 棒進出場搞混。** 面板 `FILLED` 那格後面的 `(n same-bar)`
是同一根 K 棒內完成進出場的次數 —— 1:1 的停利很近，這種交易是常態
（前瞻記錄 5 筆裡有 2 筆），腳本必須正確處理它們，那不是 bug。

真正的病徵是**兩筆交易的進場價完全相同**，或**時段外成交**。
在 v6 上還看到這兩者之一，回報給我。
...
考試和 funded 是**不同的東西**，設定也應該不同：
考試是一張 $49 的彩票（reset 同價，且月費續訂還送一張），爆了再買。
**funded 帳戶買得回來，但貴得多**：Back2Funded 是 $599（選 Daily Loss Limit
折 $50 → **$549**），而且**只限第一次出金之前**、關閉後 30 天內。
$549 對上 $49 是 11 倍，而且那張保險只能在拿到第一筆出金以前用。

| 設定 | **Combine（考試）** | **Express Funded** |
|---|---|---|
| `propMode` | on | on |
| `firmPreset` | Topstep 50K | Topstep 50K |
| `riskBasis` | % of account | % of account |
| `riskPct` | **1.8**（$50K 上是 $900/單） | **1.8**（不降） |
| `useGuard` | **off** | **off**（不開） |
| `scalePlan` | off（固定 50 micros） | **on**（20 → 30 → 50 跟餘額走） |
| `safetyMult` | — | — |

**兩欄現在只差 `scalePlan` 一項。** 2026-09-20 改的，理由在下面。

### 為什麼考試階段要把守衛關掉

這一條反直覺，所以放數字：假設樣本外的優勢是真的，
在 Topstep 50K 上跑 3,000 次考試的結果 ——

| 風險/單 | 佔 50K | 守衛 | 單次通過 | 爆倉 | 單次耗時 | **期望幾次考過** |
|---|---|---|---|---|---|---|
| $200 | 0.4% | off | 79% | 21% | 6.7 月 | 1.3 次 / **9.4 個月** |
| $500 | 1.0% | off | 45% | 55% | 1.1 月 | 2.2 次 / **2.7 個月** |
| **$1,000** | **2.0%** | **off** | **35%** | **65%** | **0.3 月** | **2.9 次 / 1.0 個月** |
| $1,000 | 2.0% | on | 10% | 0% | 0.2 月 | 10 次 / 1.7 個月 |

**風險開大反而快 9 倍、便宜 9 倍**，因為失敗很快很便宜，
而低風險的成功很慢 —— $200 那條要付九個月的月費。

守衛在這裡是反效果：它把爆倉換成「停手」，而**停手既不通過也不爆倉，
月費照樣在走**。最後一行就是代價：通過率從 35% 掉到 10%。

### ⚠ 上面那張表是 bootstrap 算的，而 bootstrap 打亂了交易順序

**這件事對這個 repo 裡每一個 bootstrap 數字都成立，不只這一張表。**

打亂順序等於假設「每次嘗試互相獨立」。實際的虧損是**聚集**的：
2025Q4 那 56 筆只有 39.3% 勝率，而且是連續的。同一段資料，
用真實順序重播（爆了就重置、從下一筆繼續）：

| | 期望次數 | 耗時 |
|---|---|---|
| bootstrap（打亂順序） | 2.9 次 | 1.0 個月 |
| **真實順序（2025Q4–2026Q3）** | **11 個帳戶** | **3.4 個月** |

差了近四倍。**不要刪掉 bootstrap 那一欄** —— 它是「若各次嘗試獨立」的上界，
仍然有用；但它是上界，不是預期值。同一個警告也適用於下面守衛開關那張
66.7% / 76.6% 的表。

可重跑：`scripts/quarterly_report.py` 的 `rebuy()`，
報告裡「Busting and re-buying」那一節。

三個必須知道的保留：

1. **這整張表建立在「2024 後的優勢是真的」這個假設上。**
   用完整 10.7 年的優勢算，通過率是 30%，而完全沒有優勢是 33%。
2. 35% 通過代表 **65% 爆倉**。期望 2.9 次是平均值，
   **有 27% 的機率要考四次以上** —— 而真實順序是 11 次。
3. **費用已經查證了**（50K Standard，2026-09-13）：月費 $49、reset $49、
   **每次月費續訂送一個免費 reset credit**、activation $149 且
   **只在拿到 funded 帳戶時收一次**。詳見 `docs/GO_LIVE.md`。

### funded 之後為什麼**不**反過來（2026-09-20 改）

這裡原本寫：funded 的目標是「不要失去」，所以風險降到 0.5%、守衛打開。
**那個推論在資料上不成立。** 2016–2026 實測（$900 進考試）：

| | 淨利 | funded 中位壽命 | funded 結局 |
|---|---|---|---|
| **$900 全程、不開守衛** | **+$44,868** | 12 筆 / 20 天 | 98% 爆倉 |
| $900 → $250、不開守衛 | +$12,983 | 127 筆 / 219 天 | 93% 爆倉 |
| $900 → $250、守衛開 | +$11,326 | 64 筆 / 111 天 | 95% 停手 |

降到 0.5% 讓帳戶活 10 倍久，但**砍掉 71% 的利潤**：倉位剩四分之一，
餘額就剩四分之一，而出金是領餘額的一半。

守衛**沒有保住帳戶**，只是把「爆倉」換成「停手」，而且死得更早。
它的判斷是 `剩餘空間 < 風險 × safetyMult`，$250 之下要等餘額低於 $375
才觸發 —— 那時候已經沒有東西可以保護。替換成本確實是 $549 對 $49，
但**開不開守衛帳戶被換掉的頻率一樣**，所以那個差別買不到東西。

上面「為什麼考試階段要把守衛關掉」那一節的論證**原封不動適用於 funded**。
完整數字和保留在 `docs/GO_LIVE.md`，重算用 `scripts/account_lifetime.py`。

### `scalePlan`：funded 的口數上限不是 50

**Express Funded 不受考試那個 50 micros 的固定上限管**，
它走 Scaling Plan：上限由餘額決定，而 XFA 從 $0 開始。
Topstep 50K 是 **$1,500 以下 2 口（20 micros）、$1,500 起 3 口、$2,000 起 5 口**。

按 50 去算倉位、實際帳戶只給 20，會在券商端被拒而圖表上看不出原因。
`scalePlan` 打開之後腳本會跟著餘額調整，每個 session 開盤刷新一次
（Topstep 也是隔日才提升，不會盤中變）。

實測這個上限對這套策略幾乎不咬：$200/單時中位數只要 3 口，
537 筆裡只有 3 筆超過 20（0.6%），代價 $42。
...
上線流程在 `docs/GO_LIVE.md`，這裡不重複。一句話版本：
**Practice Account 跑到對帳連續乾淨之前，不要買 Combine。**
那一步官方明文要求，也是唯一能在真錢之前發現路徑問題的機會 ——
而官方同時說明，程式出錯造成的交易**不受理**。
