...
**圖表左上角的圖例會顯示版本號**：`LTF Sweep v4`。
看到版本號就是新版，只看到 `LTF Sweep` 就是舊的還掛著 ——
不用開面板、不用翻 log，你截圖的時候本來就會拍到。

`debugLog` 打開的話，Pine Logs 第一行也會印 `LTF Sweep v4`。
...
`riskUSD = 500` 的樣本外最大回撤是 **$9,615**，而 Topstep 50K 的
...
**守衛把 30.9% 的「帳戶沒了」換成「停止交易但帳戶還在」**，
代價是 10.5 個百分點的通過率。這個交換很划算：爆倉要重買評估重來，
停手只是那一輪沒過。
...
|---|---|---|---|
| $100 | 437 / 250 | −$962 | 94% / 22 個月 |
| **$200** | **564 / 123** | **−$2,569** | **67% / 7 個月** |
| $300 | 581 / 106 | −$5,384 | 50% / 3 個月 |
| $500 | 581 / 106 | −$9,615 | 38% / 1 個月 |
