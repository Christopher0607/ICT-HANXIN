...
|---|---|---|
| `--risk` | **900** | **900**（不降） |
| `BRIDGE_USE_GUARD` | **0** | **0**（不開） |
| `BRIDGE_SCALING_PLAN` | 0 | **1** |
| `BRIDGE_LIVE_DATA` | 1 | 1 |
| `BRIDGE_ACCOUNT_START` | 50000 | 50000（只有守衛在用，關掉就不影響） |

**兩個階段其實是同一組設定**，只有 `BRIDGE_SCALING_PLAN` 不同 ——
funded 的口數上限不是固定 50，是跟餘額走的 20 → 30 → 50。
...
### 為什麼 funded 之後也不降風險、也不開守衛（2026-09-20 改）

這份文件原本寫 funded 要降到 $250、守衛打開，理由是「funded 帳戶貴得多，
要保住它」。**那個理由在資料上不成立**，2016–2026 實測（$900 進考試）：

| | 帳戶 | XFA | 淨利 | funded 中位壽命 | funded 結局 |
|---|---|---|---|---|---|
| **$900 全程、不開守衛** | 153 | 52 | **+$44,868** | 12 筆 / 20 天 | 98% 爆倉 |
| $900 → $250、不開守衛 | 58 | 15 | +$12,983 | 127 筆 / 219 天 | 93% 爆倉 |
| $900 → $250、守衛開 | 73 | 21 | +$11,326 | 64 筆 / 111 天 | 95% 停手 |

**降到 $250 讓帳戶活 10 倍久，但砍掉 71% 的利潤** —— 倉位剩四分之一，
餘額就剩四分之一，而出金是領餘額的一半。活得久不等於領得多。

**守衛沒有保住帳戶，只是把「爆倉」換成「停手」，而且死得更早**
（64 筆 vs 127 筆）。它的判斷是 `剩餘空間 < 風險 × 1.5`，
$250 之下要等餘額低於 $375 才觸發 —— 到那時候已經沒有東西可以保護了。

原本考試階段關守衛的理由（`tradingview/README.md`：守衛把爆倉換成停手，
而停手既不通過也不爆倉、月費照走，通過率從 35% 掉到 10%）**同樣適用於 funded**。
唯一的差別是替換成本（$49 的重置 vs Back2Funded $549），
而實測**開不開守衛，帳戶被換掉的頻率一樣**，所以那個差別買不到東西。

⚠ **模型把「停手」當成帳戶結束。** 照守衛自己的規則，停手的帳戶不交易 →
餘額不變 → 停手條件永遠成立，所以它再也不會動。
但現實裡它在 Topstep 那邊還活著，你面對的是一個選擇：關掉守衛繼續跑，
或讓它空著佔住 5 個 XFA 名額之一。**回測決定不了哪個對。**

**守衛全程關掉的附帶好處**：`BRIDGE_ACCOUNT_START` 只有守衛在讀，
關掉之後那個「換帳戶要記得同步餘額」的手動步驟就不存在了，
少一個會在第 40 天咬你的失誤點。
